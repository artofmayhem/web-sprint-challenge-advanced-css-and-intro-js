...
function getArtistByIndex(array, index) {
  return `The artist at index ${index} is ${artists[index - 1].name}`;
}
...
function get20s(newArrayToFilter)
{ 
  let get20sArray = [];
  for (let i = 0; i < newArrayToFilter.length; i++)
  {
     if  (newArrayToFilter[i][“years”] >= “1900-2000")
         {
         get20sArray.push(newArrayToFilter[i][“name”]); 
         }
  }   
  console.log(get20sArray);
  return get20sArray;
}
...
const newArtist = {
  ...
  'genre': 'Comic Book',
  'state': 'Hawaii',
  'bio': 'As a youth, Tony spent his summers reading his favorite comic books, These comic books included X-men, New Mutants, Batman, and Superman.'
}

function addArtist(object1, object2){
  let revisedList = object1;
  revisedList.push(object2);
  console.log(revisedList[20]);
  return revisedList;
};
...
function lotsOfArt(array){
  const filteredArtists = array.filter((num) => {
      return num.paintings >= 100;
  });
